      ...
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              {
                text: `Analyze this image of a household item being packed for an international move. Return ONLY a valid JSON object with no markdown, no code fences, no explanation. Use exactly this structure: {"item_name":"","item_type":"","description":"","est_value":0,"replacement_value":0,"prompt_serial":false,"confidence":"low"}. Rules: item_type must be exactly one of: ${validTypes.join(', ')}. description must be 10 words or fewer, physical attributes only (material, color, size), no brand names, no logos, example: "large red ceramic mug". est_value is the typical secondhand value in USD, replacement_value is the typical new replacement cost in USD, both must be plain numbers with no currency symbols, be conservative and realistic, a typical mug is $3-8 secondhand and $12-15 to replace. prompt_serial must be boolean true only for electronics, appliances, or items with a visible serial number. confidence must be exactly "high", "medium", or "low" based on how clearly the item is identifiable.`
              },
